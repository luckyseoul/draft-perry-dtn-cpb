...
if [ -x /usr/local/bin/dtnex ] && [ -f /home/nick/ion-dtn-dtnex/dtnex-soulkiller.conf ]; then
  pkill -x dtnex 2>/dev/null || true; sleep 1; cd /home/nick/ion-dtn-dtnex; nohup /usr/local/bin/dtnex -c dtnex-soulkiller.conf >> /tmp/dtnex-soulkiller.log 2>&1 &; echo "DTNEx soulkiller PID $!"
fi
