#!/bin/bash
# Robust start for ION node 122 on soulkiller
set -euo pipefail
NODE="122"
HOST="soulkiller"
...
echo "==> daemons..."
bpclock & ipnfw &
udpclo 100.96.108.37:4556 & udpclo 100.92.115.65:4556 & udpclo 100.65.168.37:4556 &
cfdpclock &
sleep 5
...
if [ -x /usr/local/bin/dtnex ] && [ -f /home/nick/ion-dtn-dtnex/dtnex-soulkiller.conf ]; then
  pkill -x dtnex 2>/dev/null || true; sleep 1; cd /home/nick/ion-dtn-dtnex; nohup /usr/local/bin/dtnex -c dtnex-soulkiller.conf >> /tmp/dtnex-soulkiller.log 2>&1 &
  DTNPID=$!
  echo "DTNEx soulkiller PID $DTNPID"
  sleep 2
  if ps -p $DTNPID >/dev/null 2>&1; then 
    echo "DTNEX soulkiller confirmed running"
  else 
    echo "DTNEX soulkiller not running, retrying..."; 
    nohup /usr/local/bin/dtnex -c dtnex-soulkiller.conf >> /tmp/dtnex-soulkiller.log 2>&1 & 
    echo "DTNEx soulkiller retry PID $!"
  fi
fi
