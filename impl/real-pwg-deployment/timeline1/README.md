...
- **Live injection over real ION stack**: Multiple `bputa <121B-file> ipn:268485121.64` (CFDP entity) from node-268485122 to node-268485121 while node-268485121 was reachable. Bundles reached the remote ION agent (see bplist "Queued for forwarding..." evidence in the notes).

- **Prior real deliveries with intact CPB decode** (from the completed 72-message + 32 test-nodes-only campaign):
  - 72 bundles in `cpb_72_with_my/` (3 real nodes × 3 targets × 8 fixed test payloads, with and without CPB).
...
- **Documented limitations documented** (so they can be followed up if reviewers ask):
  ...
  - Node-268485121 (268485121) had only a partial ION install (no `bprecv` binary). Direct BP listeners produced error files; successful captures used the CFDP file-delivery path + the `find /tmp ... -size 12[0-2]c -mmin -N` recipe.
...
**Note:** `cpb.py` in this directory is a copy of the reference implementation (synced to match the draft at combine time). It implements the CDDL and CBOR rules from Section 3.
...
- **Node configs & launchers**:
  - `host26848512{1,2,3}.rc` (IPN-based; 123 is node-268485123)
  - `start-dtn.sh` — **primary unified launcher** (run this from *any* node; auto-detects node-268485122/122, node-268485121/121 or node-268485123/123 via hostname or Tailscale IP, does full robust ION clean+start + explicit cfdpclock/bputa + DTNEx)
  - `start-node-268485123.sh` (node-268485123/123), `start-node-268485121.sh`, `start-node-268485122.sh` — thin wrappers that delegate to `start-dtn.sh`. `start-123.sh` is a symlink to `start-node-268485123.sh` for backward compat.
...
- **Logs & misc**:
  - `receive_logs/` (mostly bprecv-not-found traces documenting the node-268485121 tooling discovery; see `receive_logs/README.txt`)
...
3. On the target node, bring up the full DTN stack with the unified launcher (works from any of the three nodes):
   ...
   ```
   (It auto-detects by hostname/Tailscale IP, runs the pre-clean, ionstart with the correct .rc, forces cfdpclock + bputa RUNNING, and starts the right DTNEx instance. The old `start-node-268485122.sh` etc. still work as thin wrappers.)
...
See the quickref for the exact "minimum bar" evidence list, the long-emulated contact diagnosis, and the prepared followup items (shorter contacts, node-268485121 tooling completion, etc.).
...
- The snapshot is intentionally a lab notebook + artifact bundle, not a polished "clean room" package. It preserves the actual state of the real PWG Tailscale + ION runs.
