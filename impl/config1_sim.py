"""
config1_sim.py -- Deterministic local policy harness and optional simulator.

The draft's validation uses the policy helpers and CPB bridge tests from
this module. The optional simulation is a synthetic model; its parameters
are not mission ephemerides, calibrated operational estimates, or evidence
of routing-performance improvement.
...
Two local route-selection policies:

  baseline (earliest predicted arrival in this topology):
  ...
  cpb (illustrative confidence-weighted policy):
    cost = latency / confidence
    where latency = predicted_arrival − now and confidence is the
    product of independent hop-success probabilities in this model.
    Lowest cost wins. This helper does not implement CGR-UCoP or a full
    CGR/SABR routing agent. The optional rate argument is not used.

Optional confidence aging (--age-conf):
  Multiplies first-hop confidences by a smooth seasonal factor in
  [0.70, 1.0] as a synthetic time-varying disturbance.

The optional simulation is deterministic per seed. Its default horizon
is seven simulated days, with ten seeds available in the longer preset.

================================================================
SYNTHETIC MODEL PARAMETERS
================================================================

Contact periods (1200/1500/1800/2400 s):
  Distinct periods provide different contact opportunities for testing
  route selection. Windows can overlap; these schedules are synthetic.

Per-(rover, orbiter) confidences (0.78-0.96 in a Latin-square pattern):
  Values avoid near-zero denominators in the local latency/confidence
  helper and give each rover a different highest-confidence orbiter.
  The optional simulator interprets them as Bernoulli contact-success
  probabilities per window; they are not measured contact statistics.

Orbiter-relay and relay-ground confidence (0.99):
  Constant synthetic values keep most probability variation on the
  first hop. The product 0.99 * 0.99 = 0.9801 assumes independence;
  no operational link-reliability claim follows from these constants.

Contact window (600 s) and inter-hop delays (60 s):
  These are fixed test inputs for contact-window and arrival-time
  calculations, not physical delay or pass-duration measurements.

Hop retries (MAX_HOP_RETRIES default = 3; CLI --hop-retries):
  Each hop allows up to R contact-window Bernoulli trials under CRN
  (same R for both arms). Increasing the available independent trials
  changes the per-hop success probability from p to 1-(1-p)^R, subject
  to the simulation horizon. This does not establish a policy ranking.

Bundle generation (every 30 s per rover, 7-day sim):
  These defaults control synthetic traffic generation. The CPB draft
  does not recommend a telemetry interval or report results from this
  optional simulation.
================================================================
"""
...
#
# This is a synthetic retry budget, not an operational recommendation.
MAX_HOP_RETRIES = 3
...
# ---- pure cost functions (local policy tests) -------------------------

def cpb_route_cost(latency: float, confidence: float,
                   bottleneck_rate: float | None = None) -> float:
    """Illustrative confidence-weighted cost used by the local tests.
    ...
    local Configuration 1 cost.
    """
    del bottleneck_rate  # unused in this local policy
    if latency < 0.0:
...
def baseline_choose(t: float, rover: int, *, age: bool = False) -> Route:
    """Pick the route with earliest predicted arrival in this topology."""
...
def cpb_choose(t: float, rover: int, *, age: bool = False) -> Route:
    """Local confidence-weighted policy: latency/confidence.
    ...
    without changing the cost form.
    """
    candidates = all_routes_from(rover)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--battery", choices=("standard", "paper"), default="standard",
                        help="standard=3 seeds; paper=10 seeds (legacy preset name)")
    parser.add_argument(
        "--strategy",
    ...
    parser.add_argument(
        ...
        help="max contact-window trials per hop (default: module MAX_HOP_RETRIES=3). "
             "Same R for both arms; synthetic retry budget.",
    )
    parser.add_argument(
        "--sweep-hop-retries",
        type=str,
