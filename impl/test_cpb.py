...
print("\n== Section 3.4: per-path array DoS limit (>8 entries) ==")
data = {cpb.F_PATH_ENTRIES: [[i, 0.5] for i in range(9)]}
try:
    cpb.encode_cpb(data)
    fail("9 path entries should have raised")
except ValueError as e:
    ok(f"9 path entries -> ValueError ({str(e)[:50]}...)")
